...
router.post('/', auth, adminOnly, async (req,res) => {
  try{
    const { title, time, location, price, age, days, images,
      duration, capacity, accommodation, transport, cancellation, nights, language, phone } = req.body
    ...
    const r = getDb().prepare('INSERT INTO programs (title,time,location,price,age,days,images,duration,capacity,accommodation,transport,cancellation,nights,language,phone,createdAt,updatedAt) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')
    const info = r.run(title || '', time || '', location || '', price || '', age || '', daysJson, imgsJson, duration||'', capacity||'', accommodation||'', transport||'', cancellation||'', nights||'', language||'', phone||'', now, now)
...
router.put('/:id', auth, adminOnly, async (req,res) => {
  try{
    ...
    const daysJson = days && Array.isArray(days) ? JSON.stringify(days) : JSON.stringify([])
    const imgsJson = images && Array.isArray(images) ? JSON.stringify(images) : JSON.stringify([])
    getDb().prepare('UPDATE programs SET title=?,time=?,location=?,price=?,age=?,days=?,images=?,duration=?,capacity=?,accommodation=?,transport=?,cancellation=?,nights=?,language=?,phone=?,updatedAt=? WHERE id=?').run(title||'', time||'', location||'', price||'', age||'', daysJson, imgsJson, duration||'', capacity||'', accommodation||'', transport||'', cancellation||'', nights||'', language||'', phone||'', now, req.params.id)
    const row = getDb().prepare('SELECT * FROM programs WHERE id = ?').get(req.params.id)
    if(!row) return res.status(404).json({ message:'Not found' })
    row.days = row.days ? JSON.parse(row.days) : []
    row.images = row.images ? JSON.parse(row.images) : []
